...
async def add_warning(member: discord.Member, channel: discord.TextChannel, reason: str):
    ...
    embed.add_field(name="🔢 Варнов", value=f"{count}/5", inline=True)

    if count >= 5:
        await member.kick(reason="5 предупреждений")
        embed.add_field(name="🚪 Действие", value="Кик с сервера", inline=False)
        embed.color = discord.Color.dark_red()
        warnings[member.id] = 0
    elif count >= 3:
        await mute_member(member, 60, "3 предупреждения")
        embed.add_field(name="🔇 Действие", value="Мут на 1 час", inline=False)
...
async def on_message(message: discord.Message):
    ...
    if is_caps(content):
        ...
        await message.channel.send(
            f"⚠️ {message.author.mention} не пиши капсом!", delete_after=5
        )
        return
...
async def check_warns(ctx, member: discord.Member):
    count = warnings[member.id]
    embed = discord.Embed(color=discord.Color.yellow())
    embed.add_field(name="⚠️ Варны", value=f"{member.mention}: **{count}/5**", inline=False)
    await ctx.send(embed=embed)
...
async def unwarn(ctx, member: discord.Member):
    if warnings[member.id] == 0:
        ...
        return
    warnings[member.id] -= 1
    await ctx.send(f"✅ Варн снят. Теперь варнов: **{warnings[member.id]}/5**", delete_after=7)
